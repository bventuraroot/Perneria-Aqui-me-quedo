...
window.showExpirationReport = function() {
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                $('#expired-count').text(response.counts.expired || 0);
                $('#critical-count').text(response.counts.critical || 0);
                $('#warning-count').text(response.counts.warning || 0);
                $('#no-expiration-count').text(response.counts.no_expiration || 0);
