...
function preventDataTablesInitialization() {
    ...
    if (table.length) {
        ...
        if ($.fn.DataTable && $.fn.DataTable.isDataTable(table[0])) {
            ...
            console.log('DataTable destruido en tabla de productos próximos a vencer');
        }
    ...
    $('.dataTable').each(function() {
        if ($(this).attr('id') === 'expiringProductsTable' || $(this).hasClass('no-datatables')) {
            if ($.fn.DataTable && $.fn.DataTable.isDataTable(this)) {
                ...
                console.log('DataTable no autorizado destruido en tabla de productos próximos a vencer');
            }
        }
    });
}
...
function renderTable() {
    ...
    filteredProducts.forEach(product => {
        ...
        if (product.expiration_date) {
            ...
            console.log(`📅 Cálculo días restantes: ${product.expiration_date} -> ${expirationDate.toISOString().split('T')[0]} = ${daysUntilExpiration} días`);
        }
