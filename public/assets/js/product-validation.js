...
class ProductValidator {
    ...
    setupCodeValidation() {
        ...
        // Validación en tiempo real del código para editar producto
    ...
    checkFormValidation(form) {
        if (this.checkAllFormValidations(form)) {
            const submitButton = form.querySelector('button[type="submit"]');
            if (submitButton) {
            ...
            if (result.exists) {
                ...
                if (typeof Swal !== 'undefined') {
                    Swal.fire({
                        ...
                        text: 'El código del producto ya existe en el sistema. Por favor, ingrese un código diferente.',
                        confirmButtonText: 'Entendido',
                        confirmButtonColor: '#ffc107'
                    });
                }
            } else {
                // Cerrar cualquier alerta previa si está abierta
    ...
    disableSubmitButton(input) {
        ...
        if (form) {
            ...
            if (submitButton) {
                submitButton.disabled = true;
                submitButton.classList.add('disabled');
                submitButton.title = 'Complete todos los campos requeridos y valide el código';
            }
        }
    }
    ...
    checkAllFormValidations(form) {
        ...
        const isCodeValid = codeField && codeField.value.trim().length > 0 && codeField.classList.contains('is-valid');
        ...
        return isCodeValid && isNameValid && isDescriptionValid && isCfiscalValid && isTypeValid && isPriceValid;
    }
